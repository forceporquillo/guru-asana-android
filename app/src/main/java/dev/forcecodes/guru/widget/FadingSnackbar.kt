...
class FadingSnackbar(context: Context, attrs: AttributeSet) : FrameLayout(context, attrs) {
    ...
    fun show(
        ...
        val showDuration = if (indefinite) {
            ENTER_DURATION + Int.MAX_VALUE.toLong()
        } else {
            ENTER_DURATION + if (longDuration) LONG_DURATION else SHORT_DURATION
        }
    ...
    companion object {
        ...
        private const val LONG_DURATION = 3_550L
    }
}